...
class ResultadoFormState extends State<ResultadoForm>
  ...
  void initState() {
    ...
    enqueteApi.resultado(widget.enquete.id).then((resultado) {
      _tabController =
          TabController(length: resultado.questoes.length, vsync: this);

      setState(() {
        this.resultado = resultado;
        this.cores = _generateColors(resultado.questoes
            .map((q) => q.validos.length)
  ...
  List<Color> _generateColors(int count) {
    ...
    for (var i = 0; i < count; i++) {
      cs.add(Color.fromARGB(0xff, _random.nextInt(0xff), _random.nextInt(0xff),
                  ...
                  child: charts.PieChart([
                    charts.Series(
                        ...
                        colorFn: (o, i) => charts.Color.fromHex(
                            code:
                                "#${(cores[i].value & 0x00FFFFFF).toRadixString(16)}"))
                  ]),
                ),
  ...
  Container _buildLegenda(ResultadoQuestao questao) {
    return Container(
      ...
      child: Column(
        children: questao.validos
            .map((o) => _buildItemLegenda(questao: questao, opcao: o))
            .toList(),
      ),
    );
  }

  Widget _buildItemLegenda({ResultadoQuestao questao, ResultadoOpcao opcao}) {
    return Container(
      decoration: BoxDecoration(
          border: const Border(
              bottom: BorderSide(width: .5, color: Colors.black12),
